...
- **Suchfeld statt Adressfeld** (08.09.2026): Über der Chip-Liste sitzt eine Suche, die nach Symbol und Familien-Label filtert. Enter übernimmt den einzigen Treffer, Escape leert das Feld, bereits gewählte Chips bleiben unabhängig von der Suche sichtbar. Tokens per Contract-Adresse hinzuzufügen ist vorerst raus: der Contract sagt nicht, was ein Token trackt, also könnte dieser Weg die 1:1-Regel nur auf das Wort des Erstellers stützen. Begründung steht im Kopfkommentar von `AssetPicker.tsx`, falls der Weg zurückkommt.
...
- Asset-Familien (DEX-Chat 07.09.2026): Jedes Listen-Token trägt ein `family: 'USD' | 'EUR' | 'BTC' | 'ETH' | 'XLM'`-Feld. Ein Pool hält genau eine Familie: USD-Stables mit USD-Stables, EUR mit EUR, ein Native nur mit seinen Wrapped-Versionen (BTC + wBTC, nie BTC + USDC). Die Chips sind nach Familie gruppiert, der erste Pick sperrt die Familie, fremde Chips werden ausgegraut und inaktiv. Gemischte Familien sind ein `error` (blockt Deploy), keine Warnung mehr. Da nur gelistete Tokens wählbar sind, hat jedes Asset im Entwurf eine Familie, die Regel hat keine Hintertür.
