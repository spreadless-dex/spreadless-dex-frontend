...
- [ ] **Status:** teilweise erledigt (12.09.2026)
- Betrifft: `SwapWidget.tsx`, `Faucet.tsx`, Cross-Links zwischen Swap ↔ Earn ↔ Faucet
...
- Umsetzungsnotiz: Im „You pay"-Panel steht bei Balance 0 eine Faucet-Zeile an der Stelle der 25/50/75/Max-Buttons (die dort nichts zu teilen haben). Sie verlinkt auf `/faucet?token=SYMBOL`; `Faucet.tsx` liest den Parameter beim Mount und wählt das Token vor. Für Token ohne offenen Mint (SAC-gewrappt, z. B. SUSD) entfällt der Parameter und die Zeile sagt, dass der Faucet dieses Token nicht minten kann — das deckt zugleich den SUSD-Teil von #20 im Swap-Kontext ab. **Noch offen:** derselbe Zero-Balance-Hinweis im Deposit-Flow und die gegenseitigen Verweise Swap ↔ Earn.
...
- 2026-09-12 — #10 teilweise umgesetzt: Zero-Balance-Hinweis im Swap-„You pay"-Panel mit Deep-Link `/faucet?token=SYMBOL`, den `Faucet.tsx` beim Mount auswertet. Für nicht mintbare Token (openMint false) ohne Parameter und mit eigener Copy. Mit verbundener Testnet-Wallet im Browser verifiziert: PYUSD (Balance 0, mintbar) → `/faucet?token=PYUSD`, Faucet öffnet auf PYUSD statt auf FAUCET_TOKENS[0] (USDx); SUSD (Balance 0, nicht mintbar) → `/faucet` mit abweichendem Text. `tsc` clean, `astro build` clean, keine Konsolenfehler.
