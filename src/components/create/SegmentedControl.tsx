...
// Options carry an optional second line (the hint under the label).
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            style={{ color: active ? 'var(--c-text)' : 'var(--c-text-muted)' }}
          >
            <span className="block text-[13px] font-semibold leading-tight">{o.label}</span>
            {o.hint && <span className="block text-[11px] leading-tight mt-0.5 opacity-80">{o.hint}</span>}
          </button>
        )
      })}
    </div>
